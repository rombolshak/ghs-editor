...
describe('PredefinedEditionsDataService', () => {
  ...
  it('should request available editions', () => {
    ...
    const requestEditions = httpTestingController.expectOne('assets/json/predefined-editions.json');
    requestEditions.flush(['gh-test', 'test-ed']);
    const editionBase1 = httpTestingController.expectOne('assets/json/ghs-data/gh-test/label.json');
    const editionBase2 = httpTestingController.expectOne('assets/json/ghs-data/test-ed/label.json');
  ...
  it('should return edition conditions', () => {
    service.getEditionConditions('test').subscribe(data => expect(data).toEqual(['c1', 'c2']));
    const request = httpTestingController.expectOne('assets/json/ghs-data/test/base.json');
