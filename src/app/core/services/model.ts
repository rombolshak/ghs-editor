...
export class Model<T> {
  ...
  constructor(initialData: T, private immutable: boolean, sharedSubscription: boolean, private clone?: (data: T) => T) {
    this._data = new BehaviorSubject(initialData);
    this.data$ = this._data.asObservable().pipe(
      map((data: T) => (this.immutable ? (clone ? clone(data) : JSON.parse(JSON.stringify(data))) : data)),
      sharedSubscription ? shareReplay({ bufferSize: 1, refCount: true }) : map((data: T) => data)
    );
  }
  ...
  set(data: T) {
    if (this.immutable) {
      const clone = this.clone ? this.clone(data) : JSON.parse(JSON.stringify(data));
...
export class ModelFactory<T> {
  ...
  createWithConfig(config: { initialData: T; immutable: boolean; sharedSubscription: boolean; clone: (data: T) => T }) {
